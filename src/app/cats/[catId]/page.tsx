...
function AIQuizWidget({ catName, catBreed, catId, onStartAssessment }: { catName: string; catBreed: string; catId: string; onStartAssessment: () => void }) {
  ...
  /* Load cat-specific demo questions first, then try AI for personalization */
  useEffect(() => {
    let cancelled = false;

    // Step 1: use cat-specific demo questions as base (always available)
    const cat = getCatById(catId);
    if (cat?.quizQuestions?.length === 4) {
      setQuizQuestions(
        cat.quizQuestions.map((q) => ({ ...q, question: () => q.question(cat.name) }))
      );
    }

    // Step 2: attempt AI-generated questions in background for personalization
    async function loadAIQuestions() {
      try {
        ...
        if (!cancelled && data.questions?.length === 4) {
          setQuizQuestions(
            data.questions.map((q: { scenario: string; options: { label: string; score: number }[] }, i: number) => ({
              id: `ai_q${i}`,
              question: () => q.scenario,
        ...
        /* demo questions already set — no fallback needed */
      } finally {
  ...
  return (
    <Card className="border-2 border-cocoa/20 bg-white shadow-[4px_4px_0px_0px_rgba(42,29,20,1)] rounded-2xl overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-lavender to-sage px-5 py-3.5 flex items-center gap-3">
        ...
        <Badge className="bg-white/20 text-white border border-white/30 font-bold text-[10px] px-2 py-0.5 rounded-full flex items-center gap-1 flex-shrink-0">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-cocoa/60 font-medium mt-1">
                Answer 4 AI-generated questions personalized for {catName}&apos;s personality and needs — get an instant compatibility preview in 30 seconds!
              </p>
              <div className="inline-flex items-center gap-1.5 bg-amber-50 border border-amber-200 rounded-full px-3 py-1 mt-2">
                <Bot className="w-3.5 h-3.5 text-amber-600" />
                <span className="text-[11px] font-bold text-amber-700">Questions generated by Gemini AI</span>
              </div>
            </div>
            <Button
              onClick={startQuiz}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
        <div className="grid lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-8">
                ...
                <section>
                  <div className="flex items-center gap-2 mb-2">
                    <SectionTag>AI Quick Match</SectionTag>
                    <span className="text-[10px] font-bold text-amber-600 bg-amber-50 border border-amber-200 rounded-full px-2 py-0.5 flex items-center gap-1">
                      <Sparkles className="w-3 h-3" /> AI-Powered Feature
                    </span>
                  </div>
                  <AIQuizWidget
                    catName={cat.name}
